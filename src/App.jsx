...
export default function App() {
  ...
  const handleBooking = async (e) => {
    ...
    // Получаем ID пользователя прямо из Telegram
    ...
    try {
      ...
      // Отправляем сигнал боту, что запись успешна
      if (window.Telegram?.WebApp) {
  ...
  const getGoogleLink = () => {
    ...
    const formatToUTC = (date) => date.toISOString().replace(/[-:]/g, '').split('.') + 'Z';
...
{issubmitting ? "Секунду, оформляем..." : "Подтвердить бронирование"}
